...
// Интерфейс пропсов компонента RelatedAnime
interface RelatedAnimeProps {
  animeId: number; 
  initialRelatedAnime?: RelatedAnime[]; 
  relatedAnimeList?: RelatedAnime[];
}

// Преобразования для типов аниме
...
/**
 * Вспомогательная функция для получения преобразованного типа аниме
 * @param kind - Тип аниме
 * @returns Преобразованное название типа
 */
...
/**
 * Вспомогательная функция для получения года
 * @param date - Строка даты в формате ISO
 * @returns Год или null
 */
...
/**
 * Вспомогательная функция для обрезки заголовка
 * @param title - Заголовок аниме
 * @param maxLength - Максимальная длина (по умолчанию 21)
 * @returns Обрезанный заголовок
 */
const truncateTitle = (title: string, maxLength: number = 21): string => {
  if (!title) return "";
...
/**
 * Компонент для отображения связанных аниме
 * @description Отображает список связанных аниме с возможностью показать больше
 * @param {RelatedAnimeProps} props - Пропсы компонента
 */
const RelatedAnime: React.FC<RelatedAnimeProps> = React.memo(({ animeId, initialRelatedAnime }) => {
  const [showAll, setShowAll] = useState(false);
  const INITIAL_SHOW_COUNT = 8;

  // Загрузка связанных аниме через SWR
  const {
    data: relatedAnime,
    error,
    isLoading,
  ...
  // Мемоизация отображаемого списка
  const displayedAnime = useMemo(() => {
    if (!relatedAnime || relatedAnime.length === 0) return [];
    return showAll ? relatedAnime : relatedAnime.slice(0, INITIAL_SHOW_COUNT);
  }, [relatedAnime, showAll]);

  const hasMore = relatedAnime ? relatedAnime.length > INITIAL_SHOW_COUNT : false;

  if (isLoading) return <div>Загрузка связанных аниме...</div>;
  if (error || !relatedAnime || relatedAnime.length === 0) return null;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <svg width="16" height="16" viewBox="0 0 24 24" className="text-white/60">
          <path
            fill="currentColor"
            d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1M8 13h8v-2H8zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"
          />
        </svg>
          ...
          {displayedAnime.map((anime) => (
            ...
            >
              ...
              <div className="mt-3 space-y-1.5">
                <div className="text-[11px] font-medium text-white/40">{anime.relation_type}</div>
...
RelatedAnime.displayName = "RelatedAnime";
