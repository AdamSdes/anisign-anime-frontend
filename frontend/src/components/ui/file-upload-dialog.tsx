...
import { FileUploadDialogProps, CropArea } from "@/shared/types/file-upload";

/**
 * Компонент диалога для загрузки и кадрирования файлов
...
export const FileUploadDialog: React.FC<FileUploadDialogProps> = ({
  ...
  const cropStartPos = React.useRef<{ x: number; y: number } | null>(null);
  ...
  const startCropping = React.useCallback(() => {
    ...
    if (imageRef.current) {
      ...
      const scale = Math.min(containerRect.width / imgWidth, containerRect.height / imgHeight);
  ...
  const handleCrop = React.useCallback(async () => {
    ...
    const realSize = cropArea.width * scaleX;
  ...
  }, [cropArea, selectedFile, onUpload, handleClose]);
  ...
  }, [selectedFile, isCropping, handleCrop, onUpload, handleClose]);

  const removeFile = React.useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (preview) {
      URL.revokeObjectURL(preview);
    }
  ...
  const handleCropMouseDown = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation();
  ...
  const handleCropMouseMove = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
            ...
            {preview ? (
                ...
                <div className="relative w-full aspect-video rounded-lg overflow-hidden">
                  ...
                  {!isCropping && (
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all duration-200 flex flex-col items-center justify-center gap-2">
                      <p className="text-white/90 text-sm font-medium">Нажмите или перетащите для замены</p>
              ...
              >
                ...
                <div className="text-center space-y-2">
                  <p className="text-white/80 font-medium">Перетащите файл сюда или нажмите для выбора</p>
                  <p className="text-white/40 text-sm">Максимальный размер: {maxSize / (1024 * 1024)}MB</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
