...
import { Star, Clock, Film, Calendar, Tag } from "lucide-react";
import { transformValue } from "@/lib/utils/transforms";
import { genresMap } from "@/shared/data/genres";
import { mergeClass } from "@/lib/utils/mergeClass";
import { TooltipAnimeProps } from "@/shared/types/anime";

/**
 * Компонент тултипа для отображения информации об аниме
...
export function TooltipAnime({ children, anime }: TooltipAnimeProps) {
  const getGenreNames = (genreIds?: string[]): string[] => {
    if (!genreIds) return [];
    return genreIds
      .map((id) => genresMap[id]?.russian || genresMap[id]?.name || "")
      .filter(Boolean);
  };
      ...
      <Tooltip>
        ...
        <TooltipContent 
          className={mergeClass(
            "w-[380px] p-0 overflow-hidden bg-[#0A0A0A] backdrop-blur-xl",
            "border border-white/[0.03] rounded-xl shadow-xl"
          )}
        >
          <div className="p-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[13px] text-white/50 font-medium">{anime.name}</p>
            </div>
            ...
            <div className="flex items-center gap-4 text-[13px]">
              ...
              <div className="flex items-center gap-1.5 text-white/40">
                ...
                <span>{anime.episodes ? `${anime.episodes} эп.` : "??"}</span>
              </div>
            </div>
