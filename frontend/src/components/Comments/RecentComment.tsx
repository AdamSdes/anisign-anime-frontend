...
// Интерфейс пропсов компонента RecentComment
interface RecentCommentProps {
  comment: Comment; // Данные комментария
}

/**
 * Компонент недавнего комментария
 * @description Отображает карточку комментария с информацией об авторе, содержимом и аниме
 * @param {RecentCommentProps} props - Пропсы компонента
 */
export const RecentComment: React.FC<RecentCommentProps> = React.memo(
  ({ comment }) => {
    return (
...
RecentComment.displayName = "RecentComment";
