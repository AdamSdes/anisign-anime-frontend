 ...
 * @description Отображает карточку аниме с изображением, информацией и всплывающей подсказкой
 * @param {AnimeCardProps} props - Пропсы компонента
 */
export const AnimeCard: React.FC<AnimeCardProps> = React.memo(
  ({ anime, genres, priority = false }) => {
    ...
    const getYear = useMemo(
      () => (date: string) => new Date(date).getFullYear(),
      []
    );
    ...
    const truncateTitle = useMemo(
      () => (title: string, maxLength: number = 21) =>
     ...
     * @returns Название жанра на русском или английском
     */
    ...
    /**
     * Рендеринг содержимого всплывающей подсказки
     * @returns JSX-элемент с информацией об аниме
     */
