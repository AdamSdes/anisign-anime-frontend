...
// Интерфейс пропсов компонента AnimeListItem
...
/**
 * Вспомогательная функция для получения названия жанра
...
/**
 * Компонент элемента списка аниме
 * @description Отображает карточку аниме с постером, названием, жанрами и дополнительной информацией
 * @param {AnimeListItemProps} props - Пропсы компонента
 */
export const AnimeListItem: React.FC<AnimeListItemProps> = React.memo(({ anime, genres }) => {
  /**
   * Транслитерация текста с русского на латиницу
   ...
   */
  const generateAnimeUrl = useMemo(
    () => (anime: Anime) => {
  ...
  const animeUrl = generateAnimeUrl(anime);
